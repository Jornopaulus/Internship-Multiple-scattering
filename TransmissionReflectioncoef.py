...
for N in range(Nn):
    ...
    for iterator in range(0,Nf):
        ...
        LHS1 = mhu/(R_0*rho_p*omega**2)*(-3j*j1_KL*k_T_i**2 + 12*1j*dj1dr_KL/R_0)  + 3j*dj1dr_KL
        LHS2 = -mhu/(R_0*rho_p*omega**2)*(-3j*j1_KL*k_T_i**2 + 12*1j*dj1dr_KL/R_0) - 3j/R_0*j1_KL
        #LHS1 = mhu/(R_0*rho_p*omega**2)*( 12*1j*dj1dr_KL/R_0)  + 3j*dj1dr_KL
        #LHS2 = -mhu/(R_0*rho_p*omega**2)*( 12*1j*dj1dr_KL/R_0) - 3j/R_0*j1_KL
        LHS = np.array([LHS1, LHS2])
        
        
        DM = np.zeros((2,2), dtype = complex)
        
        DM[0,0] = mhu/(R_0*rho_p*omega**2)*(k_T_i**2*h1_KL + 3*S_1p*j1_KL*k_T_i**2 - 4/R_0*dh1dr_KL - 12*S_1p/R_0*dj1dr_KL) - dh1dr_KL - 3*S_1p*dj1dr_KL
        
        DM[0,1] = mhu/(R_0*rho_p*omega**2)*(-4/R_0*dh1dr_KT - 6*S_1s*dj1dr_KT/R_0 - 4*h1_KT*k_T_i**2 - 4*h1_KT/R_0 - 6*S_1s*j1_KT/R_0 - 6*k_T_i**2*S_1s*j1_KT + 4*h1_KT/R_0**2 + 6*S_1s/R_0**2*j1_KT) + 2*h1_KT/R_0 + 3/(R_0)*S_1s*j1_KT
        
        DM[1,0] = -mhu/(R_0*rho_p*omega**2)*(k_T_i**2*h1_KL + 3*S_1p*j1_KL*k_T_i**2 - 4/R_0*dh1dr_KL - 12*S_1p/R_0*dj1dr_KL) + h1_KL/R_0 + 3*S_1p/R_0*j1_KL
        
        DM[1,1] = -mhu/(R_0*rho_p*omega**2)*(-4/R_0*dh1dr_KT - 6*S_1s*dj1dr_KT/R_0 - 4*h1_KT*k_T_i**2 - 4*h1_KT/R_0 - 6*S_1s*j1_KT/R_0 - 6*k_T_i**2*S_1s*j1_KT + 4*h1_KT/R_0**2 + 6*S_1s/R_0**2*j1_KT) - h1_KT/R_0 - 3/(2*R_0)*S_1s*j1_KT - dh1dr_KT - 3/2*S_1s*dj1dr_KT

        solution = np.linalg.solve(DM,LHS)
...
for ii in range(Nn):
    ...
    plt.subplot(211)
    plt.suptitle("Transmission and reflection for "+ Flag +" , $d = R*(1+\\beta)$" , fontsize=40)
