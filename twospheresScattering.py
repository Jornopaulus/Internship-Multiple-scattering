# -*- coding: utf-8 -*-
"""
Created on Mon May 20 16:08:59 2019
...
for N in range(Nn):
    ...
    for iterator in range(0,Nf):
        ...
        DM1 = np.zeros((2,2), dtype = complex)
        ...
        RHSDM1 = np.array([T0_LL[iterator], T0_LL[iterator]])
        
        solutionA1A2  = np.linalg.solve(DM1, RHSDM1)
        
        A1[N,iterator]  = solutionA1A2[0]
        A2[N,iterator]  = solutionA1A2[1]
        
        
        DM2 = np.zeros((4,4), dtype = complex)
        DM2[0,0] = 1.0 
        DM2[0,1] = T1_LL[iterator]*(-spherical_hn1(0,k_L_i*2*d) - 4/3*spherical_hn1(2,k_L_i*2*d))
        DM2[0,2] = 0.0
        DM2[0,3] = T1_TL[iterator]*(-spherical_hn1(0,k_T_i*2*d) - 4/3*spherical_hn1(2,k_T_i*2*d))
        DM2[1,0] = T1_LL[iterator]*(-spherical_hn1(0,k_L_i*2*d) - 4/3*spherical_hn1(2,k_L_i*2*d))
        DM2[1,1] = 1.0 
        DM2[1,2] = T1_TL[iterator]*(-spherical_hn1(0,k_T_i*2*d) - 4/3*spherical_hn1(2,k_T_i*2*d))
        DM2[1,3] =  0.0 
        DM2[2,0] = 0.0
        DM2[2,1] =  T1_LT[iterator]*(-spherical_hn1(0,k_L_i*2*d) - 4/3*spherical_hn1(2,k_L_i*2*d)) 
        DM2[2,2] = 1.0
        DM2[2,3] = T1_TT[iterator]*(-spherical_hn1(0,k_T_i*2*d) - 4/3*spherical_hn1(2,k_T_i*2*d))
        DM2[3,0] = T1_LT[iterator]*(-spherical_hn1(0,k_L_i*2*d) - 4/3*spherical_hn1(2,k_L_i*2*d))
        DM2[3,1] = 0.0 
        DM2[3,2] = T1_TT[iterator]*(-spherical_hn1(0,k_T_i*2*d) - 4/3*spherical_hn1(2,k_T_i*2*d))
        DM2[3,3] = 1.0 
        
        RHSDM2 = np.array([T1_LL[iterator] , T1_LL[iterator] , T1_TT[iterator] , T1_TT[iterator]])
        solutionBC  = np.linalg.solve(DM2, RHSDM2)
        B1[N,iterator] = solutionBC[0]
        B2[N,iterator] = solutionBC[1]
        C1[N,iterator] = solutionBC[2]
        C2[N,iterator] = solutionBC[3]
        
        R[N,iterator] = 2*m.pi*(A1[N,iterator] + B1[N,iterator]*1j)/(k_L_i**2*4*d**2)
        T[N,iterator] = 1 + 2*m.pi*(A1[N,iterator] - B1[N,iterator]*1j)/(k_L_i**2*4*d**2)
...
for ii in range(Nn):
    plt.subplot(211)
    plt.suptitle("A1 and A2 , $d = R*(1+beta)$", fontsize=40)
    plt.plot(abs(f/f_res) , abs(A1[ii,:]), label=str(Beta[ii])+ "$ = beta $")
    plt.ylabel("A1", fontsize=40)
    plt.tick_params(labelsize=40)
    plt.xlabel("$f/f_{res}$", fontsize=40)
    plt.legend( fontsize=40, loc = 'best')
    plt.subplot(212)
    plt.plot(abs(f/f_res) , abs(A2[ii,:]), label=str(Beta[ii])+ "$ = beta $")
...
for ii in range(Nn):
    plt.subplot(211)
    plt.suptitle("B1 and B2, $d = R*(1+beta)$", fontsize=40)
    plt.plot(abs(f/f_res) , abs(B1[ii,:]), label=str(Beta[ii])+ "$= beta $")
    plt.ylabel("B1", fontsize=40)
    plt.tick_params(labelsize=40)
    plt.xlabel("$f/f_{res}$", fontsize=40)
    plt.legend( fontsize=40, loc = 'best')
    plt.subplot(212)
    plt.plot(abs(f/f_res) , abs(B2[ii,:]), label=str(Beta[ii])+ "$= beta $")
...
for ii in range(Nn):
    plt.subplot(211)
    plt.suptitle("C1 and C2, $d = R*(1+beta)$", fontsize=40)
    plt.plot(abs(f/f_res) , abs(C1[ii,:]),label=str(Beta[ii])+ "$= beta $")
    plt.legend( fontsize=40, loc = 'best')
    plt.ylabel("C1", fontsize=40)
    plt.tick_params(labelsize=40)
    plt.xlabel("$f/f_{res}$", fontsize=40)
    plt.subplot(212)
    plt.plot(abs(f/f_res) , abs(C2[ii,:]),label=str(Beta[ii])+ "$= beta $")
